...
object Problema11 {
	...
	def rezolvare() = {
		...
		//math.max(max_product_horizontal(sir), max_product_vertical(sir))
		max_product_diagonal_reverse(sir)
	}


	def max_product_horizontal(s: List[List[String]]): Int = {
		s.sliding(4).flatten.map(_.sliding(4).map(x => x.map(_.toInt).product)).flatten.max
	}

	def max_product_vertical(s: List[List[String]]): Int = {
		s.sliding(4).map(zip_lists(_)).flatten.map(_.map(_.toInt).product).max
	}

	def max_product_diagonal(s: List[List[String]]): Int = {
		s.sliding(4).map(zip_lists_diagonal(_)).flatten.map(_.map(_.toInt).product).max
	}

	def max_product_diagonal_reverse(s: List[List[String]]): Int = {
		max_product_diagonal(s.map(_.reverse))
	}

	def zip_lists(s: List[List[String]]): List[List[String]] = {
		if (s.head.length == 1)
			List(s.map(_.head))
		else
			List(s.map(_.head)) ++ zip_lists(s.map(_.tail))
	}

	def zip_lists_diagonal(s: List[List[String]]): List[List[String]] = {
		if (s.head.length == 4)
			List(List(s.head.head) ++ List(s.drop(1).head.drop(1).head) ++ List(s.drop(2).head.drop(2).head) ++ List(s.drop(3).head.drop(3).head))
		else
			List(List(s.head.head) ++ List(s.drop(1).head.drop(1).head) ++ List(s.drop(2).head.drop(2).head) ++ List(s.drop(3).head.drop(3).head)) ++ zip_lists_diagonal(s.map(_.tail))
	}
}
